...
class Inventario extends poolConnection{
    ...
    public function ObtenerDescripcionArticuloCABMS($xIDCABMSConmbustible) {
        ...
        $Contador = 0;
        while ($Localizacion = mysqli_fetch_array($TLocalizacion)){
    ...
    public function ObtenerLocalizaciones() {
        ...
        $TLocalizacion = $objConexion->Query($con,$StrConsulta);
        $Contador = 0;
        if (mysqli_num_rows($TLocalizacion) > 0) {
            while ($Localizacion = mysqli_fetch_array($TLocalizacion)) {
    ...
    public function ObtenerDescripcionLocalizacion($xIDLocalizacion) {
        ...
        if (mysqli_num_rows($TLocalizacion) > 0) {
            $Localizacion = mysqli_fetch_array($TLocalizacion);
    ...
    public function RegistrarTransferencia($xIDCABMS, $xClaveAC, $xClaveInternaAC, $xIDLocalizacionOrigen, $xIDLocalizacionTraspaso, $xCantidad){
        ...
        $CantidadActual = 0;
        if (mysqli_num_rows($TArticulo) > 0){
            $Articulo = mysqli_fetch_array($TArticulo);
        ...
        if ($SeReduce) {
            ...
            $TArticulo = $objConexion->Query($con,$StrConsulta);
            $TransferenciaRegistrada = mysqli_affected_rows();

...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->Ln(5);
		$this->Cell(0,5,'Movimiento al Kardex',0,0,'C');
		// Salto de línea
	...
	{
		$this->AnchoColumnas = array(30,30,30,100,25,25,40);
		$this->CabeceraTabla = array('Fecha','No.Pedido','No.Folio','Procedencia/Destino','Entrada','Salida');
		$RC = new Reporte_MovimientoKardex();
	...
	function GenerarReporte($FechaInicial,$FechaFinal)
	{
		...
		foreach($this->DatosCatalogo as $Dato)
		{
			...
			$this->Cell($this->AnchoColumnas[3],6,'Procedecian/Destino','', 0, 'L');
			$this->Cell($this->AnchoColumnas[4],6,$Dato[3],'', 0, 'C');
			$this->Cell($this->AnchoColumnas[5],6,$Dato[4],'', 0, 'C');
			$this->Ln();
		}
		$this->Output('reporte_movkarderx_pdf.pdf', 'I');
	}
}
...
$pdf->GenerarReporte($_GET[v2],$_GET[v3]);

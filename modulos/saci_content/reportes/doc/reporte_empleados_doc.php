<?php
    ...
    header("Content-Disposition: inline; filename=reporte_catalogo_empleados.doc");
    ...
    $cell->addText(utf8_decode('Reporte del Catálogo de Empleados'), array('name'=>'Arial', 'size'=>16, 'bold'=>true), array('align'=>'center'));
    $table->addRow(100);
    $cell = $table->addCell(16000);
    ...
    $table->addRow(100);
    $table->addCell(1600)->addText('No. Empleado', $styleCell, array('align'=>'center'));
    $table->addCell(4600)->addText('Nombre', $styleCell, array('align'=>'center'));
    $table->addCell(2200)->addText('RFC', $styleCell, array('align'=>'center'));
    $table->addCell(2200)->addText('Zona de Pago', $styleCell, array('align'=>'center'));
    $table->addCell(1600)->addText('Z. P. Ant.', $styleCell, array('align'=>'center'));
    $table->addCell(2200)->addText('Cargo', $styleCell, array('align'=>'center'));
    $table->addCell(1600)->addText('Estado', $styleCell, array('align'=>'center'));
    $RC = new Reporte_Catalogos();
    $Datos = $RC->ObtenerArregloCatalogoEmpleados();
    $i = 1;
    foreach ($Datos as $F) {
        $table->addRow(100);
        $table->addCell(1600)->addText($F[0], array(), array('align'=>'center'));
        $table->addCell(4600)->addText($F[1]);
        $table->addCell(2200)->addText($F[2], array(), array('align'=>'center'));
        $table->addCell(2200)->addText($F[3], array(), array('align'=>'center'));
        $table->addCell(2200)->addText($F[4], array(), array('align'=>'center'));
        $table->addCell(1600)->addText($F[5], array(), array('align'=>'center'));
        $table->addCell(1600)->addText($F[6], array(), array('align'=>'center'));
    }

<?php
    ...
    header("Content-Disposition: inline; filename=reporte_catalogo_cabms.doc");
    ...
    $cell->addText(utf8_decode('Reporte del Catálogo de CABMS'), array('name'=>'Arial', 'size'=>16, 'bold'=>true), array('align'=>'center'));
    $table->addRow(100);
    $cell = $table->addCell(16000);
    ...
    $table->addRow(100);
    $table->addCell(1600)->addText('ID', $styleCell, array('align'=>'center'));
    $table->addCell(1600)->addText('IDCABMS', $styleCell, array('align'=>'center'));
    $table->addCell(1600)->addText('ID_UMedida', $styleCell, array('align'=>'center'));
    $table->addCell(3600)->addText(utf8_decode('Descripción'), $styleCell, array('align'=>'center'));
    $table->addCell(2200)->addText('Tipo de Almacen', $styleCell, array('align'=>'center'));
    $table->addCell(2200)->addText('Consecutivo Inv.', $styleCell, array('align'=>'center'));
    $table->addCell(2200)->addText('Partida Presupuestal', $styleCell, array('align'=>'center'));
    $RC = new Reporte_Catalogos();
    $Datos = $RC->ObtenerArregloCatalogoCABMS();
    $i = 1;
    foreach ($Datos as $F) {
        $table->addRow(100);
        $table->addCell(1600)->addText($F[0], array(), array('align'=>'center'));
        $table->addCell(1600)->addText($F[1], array(), array('align'=>'center'));
        $table->addCell(1600)->addText($F[2], array(), array('align'=>'center'));
        $table->addCell(3600)->addText($F[3]);
        $table->addCell(2200)->addText($F[4], array(), array('align'=>'center'));
        $table->addCell(2200)->addText($F[5], array(), array('align'=>'center'));
        $table->addCell(2200)->addText($F[6], array(), array('align'=>'center'));
    }

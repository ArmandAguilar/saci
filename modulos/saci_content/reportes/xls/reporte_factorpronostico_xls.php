<?php
    ...
    header("Content-Disposition: attachment; filename=reporte_catalogo_factor.xls");
    ...
    ->setKeywords("")
    ->setCategory("");
    $PHPExcel->getActiveSheet()->setTitle('FACTOR PRONOSTICO');
    ...
    $PHPExcel->getActiveSheet()->mergeCells('A1:G1')->setCellValue('A1', 'REPORTE DE CATALOGO DE FACTOR PRONOSTICO')->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    ...
    $PHPExcel->getActiveSheet()->getStyle('A7:D7')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    $PHPExcel->getActiveSheet()->getStyle('A7:D7')->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);
    ...
    ->setCellValue("B7", "Año")
    ->setCellValue("C7", "Mes")
    ->setCellValue("D7", "Factor");
    $RC = new Reporte_Catalogos();
    $Datos = $RC->ObtenerArregloCatalogoFactorPronostico();
    $i = 8;
    foreach ($Datos as $F) {
        $PHPExcel->setActiveSheetIndex(0)

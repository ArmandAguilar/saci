<?php
    ...
    header("Content-Disposition: attachment; filename=reporte_catalogo_parametros.xls");
    ...
    ->setKeywords("")
    ->setCategory("");
    $PHPExcel->getActiveSheet()->setTitle('CAT. DE PARAMETROS');
    ...
    $PHPExcel->getActiveSheet()->mergeCells('A1:G1')->setCellValue('A1', 'REPORTE DE CATALOGO DE PARAMETROS')->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    ...
    ->setCellValue("A7", "Id_Parametro")
    ->setCellValue("B7", "Descripcion")
    ->setCellValue("C7", "Valor");
    
    $RC = new Reporte_Catalogos();
    $Datos = $RC->ObtenerArregloCatalogoParametro();
    $i = 8;
    foreach ($Datos as $F) {
        $PHPExcel->setActiveSheetIndex(0)

<?php
    ...
    header("Content-Disposition: attachment; filename=reporte_catalogo_tipobien.xls");
    ...
    ->setKeywords("")
    ->setCategory("");
    $PHPExcel->getActiveSheet()->setTitle('CAT. TIPO BIEN');
    ...
    $PHPExcel->getActiveSheet()->mergeCells('A1:G1')->setCellValue('A1', 'REPORTE DE CATALOGO DE TIPO DE BIEN')->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    ...
    ->setCellValue("A7", "Id TipoBien")
    ->setCellValue("B7", "Descripcion");

    $RC = new Reporte_Catalogos();
    $Datos = $RC->ObtenerArregloCatalogoTipoBien();
    $i = 8;
    foreach ($Datos as $F) {
        $PHPExcel->setActiveSheetIndex(0)
